...
int main()
{
	...
	fp = fopen("pqr.txt", "r");
	if(error_check(fp, "reading in main()", FILE_OPENING_FAILURE) == SUCCESS)
    {                                                                           
        return(-1);                                                        
    }  
    ...
    for (i = 0; i < 10; ++i)
    {
        ret = pthread_create(&thread_ids[i], NULL, thread_function, (void *)p_list);
        assert(ret == 0);
    }
